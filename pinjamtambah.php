...
$query_petugas = "SELECT * FROM tbl_petugas WHERE username='".$_SESSION['username'] . "' LIMIT 1";
...
<body>
    ...
    <form action="halaman/pinjam/pinjamtambah_aksi.php" method="post">
        <table>
            ...
            <tr>
                <td>Nama Petugas</td>
                <td><input readonly type="text" value="<?php echo $data['namapetugas']; ?>" readonly></td>
            </tr>
             <tr>
                <td>Nama Siswa</td>
                <td><select name="siswa">
                    ...
                    while($row = mysqli_fetch_assoc($data_siswa)) {
                        echo "<option value=". $row['idsiswa'] .">". $row['namasiswa'] ."</option>";
                    }
                    ?>
                </select>
            </td>
